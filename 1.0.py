...
APPNAME = "Fxxk-Y!mobile-Apps"
VERSION = 1.0
DEVELOP = "@wakanameko2"

APPSID = [  # 対象アプリケーションID (e.g. com.example.myapp) のリスト
    "jp.co.yahoo.android.premiumwebclick",         #Enjoyパック
    "jp.co.yahoo.android.apps.navi",               #Y!カーナビ
    "jp.co.yahoo.android.ymobile.mail",            #Y!mobile メール
    "jp.co.yahoo.android.ybrowser",                #Y!ブラウザー
    "jp.co.yahoo.android.ybox",                    #Y!ボックス
    "jp.ymobile.android.myymobile",                #My Y!mobile
    "jp.co.yahoo.android.yjtop",                   #Yahoo!
    "jp.co.yahoo.android.YAuctionPad",    #ヤフオク!
]

icodata = '''R0lGODlhAAEAAffgAAAAADY2Njk5OTc3ODAwLjExVldXV15dXFhYV0RERFxbYlpa
...
#########
baseGround = tk.Tk()
baseGround.geometry('600x430')
baseGround.resizable(width = False, height = False)
baseGround.title(APPNAME + " | ver." + str(VERSION) + " | Developer:" + DEVELOP)
baseGround.tk.call('wm', 'iconphoto', baseGround._w, tk.PhotoImage(data=icodata))
...
def btn_click():
    for i in range(0, len(APPSID)):
        ...
        print("Deleted the " + APPSID[i])
        i += 1

#########
# window widgets
#########
#Label_icon = tk.Label(baseGround, image = IMGicodata)
Label_AppnameLavel = tk.Label(baseGround, text = f"{APPNAME}", font = ("normal", 12, "bold"))
button = tk.Button(
    baseGround, text='実行', command=btn_click).place(x= 10, y=10)
